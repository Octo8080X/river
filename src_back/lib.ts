...
// Helper to determine if a result is a promise
function isPromise<T>(value: any): value is Promise<T> {
  return value && typeof value.then === "function";
}

// Helper function to create async Result
export function asyncResult<T, F>(
  promise: Promise<Result<T, F>>,
): Result<T, F> {
  return promise as unknown as Result<T, F>;
}
...
export function pipeline(...fns: Array<(arg?: any) => Result<any, any>>): any {
  ...
  return (
    ...
    for (let i = 1; i < fns.length; i++) {
      ...
      // Handle async result
      if (isPromise(result)) {
        return handleAsyncPipelineFromIndex(
          fns,
          i,
          currentValue,
          errorHandler,
...
async function handleAsyncPipeline(
  fns: Array<(arg?: any) => Result<any, any>>,
  errorHandler?: (
...
// Handling asynchronous pipeline from a specific index
async function handleAsyncPipelineFromIndex(
  fns: Array<(arg?: any) => Result<any, any>>,
  startIndex: number,
  initialValue: any,
  errorHandler?: (
